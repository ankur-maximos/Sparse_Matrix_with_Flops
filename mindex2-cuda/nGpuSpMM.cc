/* Main class invoking sparse matrix multiplication */
...
bool resultsComparison(CSR &hC, CSR &rC, const vector<int> &hv, const int *hqueue) {
  ...
  assert (hv.size() == 65);
  printf("Checking fp>1024 olarge nonzero entries in single A's row  count= %d\n", hv[64] - hv[63]);
    printf("hC VS rC\n");
...
CSR scudaSpMM(const CSR &hA, const CSR &hB) {
  ...
  int *dqueue = NULL;
  int *dflops = NULL;
  vector<int> hv = gpuFlopsClassify(dA, dB, &dqueue, &dflops);
  // int *flops;
  //  HANDLE_ERROR(cudaMemcpy(flops,dflops,4,cudaMemcpyDeviceToHost));
  timer t2;
  CSR hC;
  //for counting flops per row for bins
  //count_row_flops(dA, dB, dqueue, hv);
  CSR dC = sgpuSpMMWrapper(dA, dB, dqueue, hv, dflops);
