...
void outputStats(const std::vector<int> &stats) {
  ...
  for (i = 0; i < stats.size() - 1; ++i) {
    const long bound = (1l << i);
    printf("(%ld -> %ld)\t%d\t%.6f\t\n", bound / 2 + 1, bound, stats[i], (Value)stats[i] / sum);
  }
  const long bound = (1l << i);
  printf("(%ld -> INF)\t%d\t%.6lf\t\n", bound / 2 + 1, stats[i], (Value)stats[i] / sum);
}

std::vector<int> flopsStats(const int IA[], const int JA[], const Value A[], const int nnzA,
